...
export const PROJECT_FILES: ProjectFile[] = [
  ...
  {
    filename: 'perfect-feed.py',
    size: '2.1K',
    date: 'jul 23',
    desc: {
      en: 'Daily news digest: RSS in, local AI translation and summary out, plus DOP rates.',
      es: 'Boletín diario de noticias: entra RSS, sale traducción y resumen con IA local, más tasas DOP.',
      ja: '毎日のニュースダイジェスト:RSSを取り込み、ローカルAIで翻訳・要約、DOP相場も掲載。',
    },
    stack: 'Python · RSS · Local LLM · GitHub Pages',
    url: 'mkawashiro2000.github.io/perfect-feed',
    code: [
      'def construir_boletin(fuentes: list[str]) -> str:',
      '    """Agrega, traduce y resume los feeds del día."""',
      '    articulos = []',
      '    for url in fuentes:',
      '        for entrada in feedparser.parse(url).entries[:LIMITE]:',
      '            if ya_publicado(entrada.id):',
      '                continue',
      '            articulos.append(Articulo(',
      '                titulo=traducir(entrada.title),',
      '                resumen=resumir_local(entrada.summary),',
      '                fuente=url,',
      '            ))',
      '',
      '    tasas = tasas_del_dia()  # USD y EUR en República Dominicana',
      '    return render("index.html", articulos=articulos, tasas=tasas)',
    ],
  },
  {
